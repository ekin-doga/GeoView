...
try
    ...
    if cfg.triggers.enabled
        ViewPixxTriggerGenerater(window, cfg.triggers.experiment_start);
        Screen('Flip', window);
        WaitSecs(0.010);
    ...
    show_attn_check_demo_generater(window, screen_cfg, cfg, instructions);
    ...
    for trial = start_trial:height(Info)
        ...
        if trial > 1 && Info.block(trial) ~= Info.block(trial - 1)
            ...
            break_text = sprintf([...
                ...
                completedBlock);
            show_attn_check_demo_generater(window, screen_cfg, cfg, break_text, 'Press SPACE to continue.');
        ...
        if cfg.triggers.enabled
            ViewPixxTriggerGenerater(window, cfg.triggers.fix_onset);
        end
        vbl = Screen('Flip', window);
        ...
        if cfg.triggers.enabled
            ViewPixxTriggerGenerater(window, Info.trigger_id(trial));
        end
    ...
    if cfg.triggers.enabled
        ViewPixxTriggerGenerater(window, cfg.triggers.experiment_end);
        Screen('Flip', window);
        WaitSecs(0.010);
        ViewPixxTriggerGenerater(window, 0);
        Screen('Flip', window);
    ...
    show_break_generater(window, end_message, cfg.deviceIndex, cfg.triggers);
